...
//        env.put("LC_ALL", localeAndCharset);
    ...
    private void runRCode() throws ExecutionException {
        ...
        if (returnCode != 0) {
            throw new ExecutionException("R command evaluating " + rSourceFile.getAbsolutePath() + " failed with error. Return code" + returnCode + ", Reason: " + errorMessageSaver.getMessage());
        }
    }
